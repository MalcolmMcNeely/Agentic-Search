...
- [account/teams.md](account/teams.md) — the current comp for each purpose, with formation, artifact, status and known weak points, plus the rules for replacing one — read when the question asks what is fielded now, and at the reconcile step of the `team-comp` skill, which drafts a comp first and compares it here after.
...
- [mechanics/team-building.md](mechanics/team-building.md) — the ordered method for building a five-hero comp, and the checks to run before handing one over — read whenever a team comp is drafted.
- [mechanics/team-archetypes.md](mechanics/team-archetypes.md) — the comp patterns a team is built on (nuke, scaling carry, ramp carry, boss amplify, Ultimate engine, summon stack, sustain, mitigation, control), the engine behind each, a matchup table, and every owned hero placed in at least one of them — read when the question asks which heroes work together, why a hero is good in one fight and bad in another, or what to build toward next.
- [mechanics/hero-scoring.md](mechanics/hero-scoring.md) — how to rank a hero from its kit alone with no guide: the two sums a hero pays into, what counts as survival, what counts as damage, ramp, and why a guide that never names a hero proves nothing about it — read when a hero has no guide coverage, when two heroes are being compared for a slot, or when a comp needs judging on whether it survives long enough to kill.
...
- [mechanics/game-modes.md](mechanics/game-modes.md) — every mode, its unlock, where to find it in the game UI, the 1125-stage ceiling on the Starter Story ladder and the uncapped Infinite Stages that follow it, and how each mode's rules change team building — read when the question names a mode or an in-game event, asks where a mode is, or asks what comes after AFK Stage 1125.
...
- [mechanics/arena.md](mechanics/arena.md) — ranked PvP: the five-tier rank ladder and its point bands, the disputed top bracket, the 5 free plus 5 bought daily tickets, where the mode sits in the Everlight, and what it asks of a comp — read when the question names Arena, an Arena rank, Arena Points, Arena tickets or the Arena Store.
- [mechanics/darkswarm-invasion.md](mechanics/darkswarm-invasion.md) — the Homestead invasion event: Aggression, Hypofiends, the 24-hour loot window, damage-based weekly contribution, the assist period and what a team needs — read when the question names Darkswarm Invasion, Hypofiends, Homestead or Aggression.
...
- [mechanics/skyclops.md](mechanics/skyclops.md) — the Skyclops Dream Realm boss, its Wings of Light and Wings of Darkness resistance flip, the damage-type rule it forces and the artifact to bring — read when the question names Skyclops or asks for a Dream Realm team.
...
- [mechanics/progression-systems.md](mechanics/progression-systems.md) — Resonating Hall, Resonance Level, Hands of Resonance, the free swap rule and how the level travels with the slot, the ten ascension tiers and what each unlocks — read when the question is about levelling, ascending, who should hold a Hand of Resonance slot, or what a tier gives.
...
- [mechanics/diamonds.md](mechanics/diamonds.md) — every diamond sink with its price and cap, the spend order, and what never to buy — read when the question is about diamonds, the Guild Store, the Emporium, Invite Letters, Stellar Crystals, Arena tickets, Quick AFK, or whether to spend or bank.
