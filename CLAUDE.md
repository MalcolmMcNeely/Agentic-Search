...
- **Score the kit, and score the whole roster.** Guides lag the game by a patch, so a hero they never mention sits outside their scope. Rank every hero owned from its own kit before any tier letter. See [mechanics/hero-scoring.md](mechanics/hero-scoring.md).
- **A tier letter carries its mode, its dated source and its investment level, or it does not go in.** Tier lists rank for one purpose, and a farming list and a PvP list disagree by design. A letter with no mode on it is an average over fights I may never take. Prefer a source inside 6 months, reject anything over 12, and check it against the vetted list in [mechanics/tier-list-sources.md](mechanics/tier-list-sources.md), which also names the sites that must never be cited.
- **Check the tier gate before crediting a skill.** A guide describes a hero at its ceiling. Legendary+ and Mythic+ each unlock a skill, so a hero below its breakpoint is a different hero. Compare the unlock tier against `account/roster.md`.
