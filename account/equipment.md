...
**Tank now carries two of the five heroes in the push comp.** Daimon and Phraesto are both Tank, so a level bought there pays twice on the comp being played, and **Tank Gloves at 185 is the weakest slot on the account**, which makes it the highest-value slot twice over. Elijah & Lailah are the one Support left in that comp. That does not overturn the rule above: the 6-level spread is still too small to pick or drop a comp, and this only reorders which class to raise next. See [teams.md](teams.md).
